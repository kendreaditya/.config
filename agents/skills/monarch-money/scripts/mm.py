...
def _smoke_probe():
    """doctor probe: confirm a saved session exists before claiming health."""
    import pathlib as _pl
    _sess = _pl.Path(__file__).resolve().parent.parent / "state" / "session.json"
    if not _sess.exists():
        raise RuntimeError("no saved session; run `mm doctor` to authenticate")
