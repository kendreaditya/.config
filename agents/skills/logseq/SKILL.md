...
Config file: `~/.config/agents/skills/logseq/state/config.json` (chmod 600). Written by `init` and `config-set`.
...
python3 ~/.config/agents/skills/logseq/scripts/tests/seed.py --reset

# Run the full test harness (73 assertions)
bash ~/.config/agents/skills/logseq/scripts/tests/run_tests.sh
```
