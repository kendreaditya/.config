#!/usr/bin/env python3
"""Shared utilities for all TikTok scripts: session management, data extraction, output."""
...
async def enrich_video(data, api=None, fetch_transcripts=True, fetch_details=False):
    """Optionally fetch detail endpoint and transcripts for a video."""
    if fetch_details and api:
        try:
    ...
    if fetch_transcripts and (data["_caption_infos"] or data["_subtitle_infos"]):
        transcript = await fetch_captions(data["_caption_infos"], data["_subtitle_infos"])
        if transcript:
...
def format_video_results(results, verbose=False):
    """Format video results for human-readable output."""
    lines = []
...
@asynccontextmanager
async def tiktok_session():
    """Create a TikTokApi session with Playwright."""
...
def output_results(results, args, format_fn=None):
    """Handle JSON/file/human output for results."""
    if format_fn is None:
        format_fn = format_video_results
    ...
    if getattr(args, 'json_output', False) or getattr(args, 'output', None):
        ...
        print(format_fn(results, verbose=verbose) if callable(format_fn) else json.dumps(results, indent=2))


def add_common_args(parser):
    """Add shared CLI flags."""
    parser.add_argument("-n", "--count", type=int, default=10, help="Number of results (default: 10)")
    parser.add_argument("--json", action="store_true", dest="json_output", help="Output as JSON")
    parser.add_argument("--verbose", action="store_true", help="Show full transcripts / details")
    parser.add_argument("-o", "--output", help="Save JSON results to file")


def add_video_args(parser):
    """Add flags specific to video-returning endpoints."""
    parser.add_argument("--no-transcripts", action="store_true", help="Skip downloading transcripts")
    parser.add_argument("--details", action="store_true", help="Also fetch detail endpoint per video (slower)")
